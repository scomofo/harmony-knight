...
const CHAPTER_3: LessonBody[] = [
  ...
  },
  {
    ...
    learn: [
      ...
      {
        kind: "text",
        body: "Compound meter is the twist: the top number counts the small pulses, not the big beats. In 6/8 there are six eighth-note pulses, but they group into two big beats — each a dotted quarter note splitting into three eighths.",
      },
      {
        kind: "text",
        body: "Rule of thumb: in simple meter the top number is the beats; in compound meter, beats = top number ÷ 3.",
      },
      {
        kind: "listen",
...
const CHAPTER_4: LessonBody[] = [
  {
    ...
    learn: [
      ...
      {
        kind: "text",
        body: "Start on any note and follow the same whole/half pattern to build that key's major scale. Walk it slowly from G: G→A is a whole step, A→B whole, B→C half, C→D whole, D→E whole.",
      },
      {
        kind: "text",
        body: "Now E→F is only a half step — but the pattern demands a whole step here. So we sharpen F to F#, and F#→G gives the final half step. The pattern forces exactly one sharp: G major is G A B C D E F# G.",
      },
      {
        kind: "listen",
  ...
  },
  {
    ...
    learn: [
      ...
      {
        kind: "text",
        body: "Two shortcuts. For sharps: the LAST sharp in the signature is always ti — one half-step below do. One sharp is F#; a half-step up from F# is G, so one sharp means G major.",
      },
      {
        kind: "text",
        body: "For flats: the SECOND-TO-LAST flat names the key — two flats are Bb and Eb, so the key is Bb major. (The lone exception: a single flat is F major.)",
      },
    ],
    checks: [
      {
  ...
  },
  {
    ...
    checks: [
      ...
      },
      {
        ...
        choices: ["Add passing color between chord tones", "Weaken the underlying harmony", "Force a modulation", "Replace the chord tones"],
        answerIndex: 0,
    ...
    checks: [
      {
        ...
        choices: ["One voice holds while the other moves", "Both voices move in the same direction", "Both voices move in opposite directions", "The voices cross above each other"],
        answerIndex: 0,
    ...
    checks: [
      {
        ...
        choices: ["Steps away and back to the same note", "Leaps away and never returns", "Resolves to a new chord tone", "Ends the phrase on a high note"],
        answerIndex: 0,
  ...
  },
  {
    ...
    checks: [
      ...
      },
      {
        ...
        choices: ["Extra pull toward their target chord", "A permanent modulation", "Weakened harmonic motion", "Atonal ambiguity"],
        answerIndex: 0,
...
const CHAPTER_9: LessonBody[] = [
  {
    ...
    checks: [
      ...
      },
      {
        ...
        choices: ["One note: B vs Bb", "One note: G vs Gb", "The root: C vs Cb", "Two chord tones"],
        answerIndex: 0,
    ...
    checks: [
      {
        ...
        choices: ["Three pulses against two in the same span", "Three bars of 2/4 time", "Two triplets in a row", "A tempo three times as fast"],
        answerIndex: 0,
  ...
  },
  {
    ...
    checks: [
      ...
      },
      {
        ...
        choices: ["Pass through dissonance between beats", "Begin on any scale degree", "Leap freely between phrases", "End on the dominant"],
        answerIndex: 0,
    ...
    checks: [
      {
        ...
        choices: ["The subject transposed to the dominant", "The subject in the relative minor", "The subject played in retrograde", "A countersubject in thirds"],
        answerIndex: 0,
    ...
    checks: [
      {
        ...
        choices: ["Transforms it via sequence, inversion, fragmentation", "Repeats it exactly each time", "Restates it louder each time", "States it once then abandons it"],
        answerIndex: 0,
    ...
    checks: [
      {
        ...
        choices: ["A raised sixth", "A raised seventh", "A lowered second", "A raised fourth"],
        answerIndex: 0,
  ...
  },
  {
    ...
    checks: [
      ...
      },
      {
        ...
        choices: ["The rhythm, mirroring the contour", "Every pitch exactly the same", "The key but not the shape", "The contour but not the rhythm"],
        answerIndex: 0,
